...
#include <ti/alg/mmwavelib/test/dssgenerated/ti_drivers_config.h>
#include <ti/alg/mmwavelib/test/dssgenerated/ti_board_config.h>
							  ...
							  uint32_t cfardBc1 )
{
	...
	for ( j = 0; j < refNumObject; j++ )
	{
		...
		if ( refRangeBin[j] >=12 && refRangeBin[j] < (numRangeBins-12) )
		{
			...
			noiseAvg = temp32 >> (c2oneside+1); /* avg noise */
		}
		else if (refRangeBin[j] < 12)
		{
			...
			noiseAvg = temp32 >> c2oneside; /* avg noise */
		}
...
#endif
		gap = h_log2Abs[refRangeBin[j]]-noiseAvg;
		printf("CA: peak= %d, avg noise= %d, gap= %d.\n",
			h_log2Abs[refRangeBin[j]], noiseAvg, gap );
		...
		/* Test CfarCadB with noise output: */
		numDetObjTemp = mmwavelib_cfarCadB_wNoise( h_log2Abs, h_detObjIdxTemp,
										numRangeBins,
										gap-1, c2oneside+1,
										guardlen, noiselen, h_noiseOut);
		if ( h_noiseOut[0] == noiseAvg )
		{
			printf("PASS: noise output of mmwavelib_cfarCadB_wNoise\n");
		}
		else
		{
			testFailed++;
			printf("FAILED: noise output of mmwavelib_cfarCadB_wNoise %d\n", h_noiseOut[0] );
		}
		...
		{	/* CA-SO: */
			...
			/* Test CfarCadB CA-SO with noise output: */
			numDetObjTemp = mmwavelib_cfarCadB_SOGO_wNoise( h_log2Abs, h_detObjIdxTemp,
											numRangeBins,
											CFAR_CASO,
											gap-1, c2oneside+1,
											guardlen, noiselen, h_noiseOut);
			if ( h_noiseOut[0] == noiseAvg )
			{
				printf("PASS: noise output of mmwavelib_cfarCadB_SOGO_wNoise\n");
			}
			else
			{
				testFailed++;
			...
			/* Test CfarCadB CA-GO with noise output: */
			numDetObjTemp = mmwavelib_cfarCadB_SOGO_wNoise( h_log2Abs, h_detObjIdxTemp,
											numRangeBins,
											CFAR_CAGO,
											gap-1, c2oneside+1,
			...
			{
				testFailed++;
				printf("FAILED: noise output of mmwavelib_cfarCadB_SOGO_wNoise %d\n", h_noiseOut[0] );
			}
	...
	for ( j = 0; j < refNumObject; j++ )
	{
			...
			noiseAvg = temp32 >>4; /* avg noise */
			gap = h_sumAbsPtr[refDopBin[j]]-noiseAvg;
			printf("peak= %d, avg noise= %d, gap= %d, threshold %d.\n",
					h_sumAbsPtr[refDopBin[j]], noiseAvg, gap, cfardBc1 );
			...
			numDetObjTemp = mmwavelib_cfarCadBwrap_wNoise(h_sumAbsPtr, h_detObjIdxTemp, numDopplerBins,
														  gap-1,4,guardlen,noiselen,
														  h_noiseOut);
			if ( h_noiseOut[0] == noiseAvg )
				printf("PASS: noise output of mmwavelib_cfarCadBwrap_wNoise\n");
			else
			{
				testFailed++;
				printf("FAILED: noise output of mmwavelib_cfarCadBwrap_wNoise %d\n", h_noiseOut[0] );
			}
...
void test_cfarnoiseoutput()
{
	int32_t    i, j, binIdx, len;
	uint32_t   c2oneside, noiselen, guardlen;
	uint16_t   *data;
	uint16_t   *detObjIdxOut, *detObjIdxRef;
	uint16_t   *noiseOut, *noiseRef;
	uint32_t   numDetObj=0, numDetRef=0;
	uint32_t   temp32, sumRight, sumLeft;
	uint16_t   noiseAvg;
	int32_t    typeId, testTypes[NUM_TYPES] = {TEST_CACA, TEST_CAGO, TEST_CASO, TEST_CAWRAP, TEST_CA};
	...
	len			= noiselen *4;
	data		= &h_sumAbs[0];
	noiseOut	= &h_sumAbs[len];
	noiseRef	= &h_sumAbs[2*len];
	detObjIdxOut = &h_sumAbs[3*len];
	detObjIdxRef = &h_sumAbs[4*len];
	...
	printf("Test CFAR with noise output functionality:\n");

	/* test all 5 types of cfar functions with noise output */
	for ( j = 0; j < NUM_TYPES; j++ )
	{
		typeId = testTypes[j];
		numDetRef=0;

		/* Reference calculation of the avgerage noise for each bin */
		for ( binIdx = 0; binIdx < len; binIdx++ )
		{
			temp32 = 0;
			sumRight = 0;
			sumLeft  = 0;
			if ( binIdx >= (noiselen+guardlen) && binIdx < (len-(noiselen+guardlen)) ) /* middle section*/
			{
				for ( i = 1; i <= noiselen; i++)
				{
					sumRight += data[ binIdx + guardlen +i ];
					sumLeft  += data[ binIdx - guardlen -i ];
				}
				if (typeId == TEST_CASO)
				{
					temp32 = (sumLeft < sumRight) ? sumLeft:sumRight; /* smaller side */
					noiseAvg = temp32 >> c2oneside; /* avg noise */
				}
				else if (typeId == TEST_CAGO)
				{
					temp32 = (sumLeft > sumRight) ? sumLeft:sumRight; /* larger side */
					noiseAvg = temp32 >> c2oneside; /* avg noise */
				}
				else
				{
					temp32 = sumRight + sumLeft; /* noise from both sides */
					noiseAvg = temp32 >> (c2oneside+1); /* avg noise */
				}
			}
			else if (binIdx < (noiselen+guardlen)) /* beginning section */
			{
				for ( i = 1; i <= noiselen; i++)
				{
					temp32 += data[ binIdx + guardlen +i ];
				}
				if (typeId == TEST_CAWRAP)
				{
					for ( i = len+binIdx-(noiselen+guardlen); i < len+binIdx-guardlen; i++)
					{
						temp32 += data[ i&(len-1) ];
					}
					noiseAvg = temp32 >> (c2oneside+1); /* avg noise */
				}
				else /* no wrap, only one side noise */
				{
					noiseAvg = temp32 >> c2oneside; /* avg noise */
				}
			}
			else /* ending section */
			{
				...
				if (typeId == TEST_CAWRAP)
				{
					for ( i = 1; i <= noiselen; i++)
					{
						temp32 += data[ (binIdx +guardlen +i) & (len-1) ];
						noiseAvg= temp32 >> (c2oneside+1); /* avg noise */
					}
				}
				else/* no wrap, only one side noise */
			...
			if (data[binIdx] > noiseAvg)
			{
				detObjIdxRef[numDetRef] = binIdx;
				noiseRef[numDetRef] = noiseAvg;
				numDetRef++;
			}
		}/* reference calculation for binIdx 0 to len*/

		switch(typeId) {
		case TEST_CAWRAP:
			{
				printf("Test mmwavelib_cfarCadBwrap_wNoise.\n");
				numDetObj = mmwavelib_cfarCadBwrap_wNoise(
													data, detObjIdxOut, len,
													0, c2oneside+1, guardlen, noiselen,
													noiseOut);
			}
			break;
		case TEST_CASO:
			{
				printf("Test mmwavelib_cfarCadB_SOGO_wNoise, CFAR_CASO.\n");
				numDetObj = mmwavelib_cfarCadB_SOGO_wNoise( 
													data, detObjIdxOut, len,
													CFAR_CASO,
													0, c2oneside+1, guardlen, noiselen,
													noiseOut);
		...
		case TEST_CAGO:
			{
				printf("Test mmwavelib_cfarCadB_SOGO_wNoise, CFAR_CAGO.\n");
				numDetObj = mmwavelib_cfarCadB_SOGO_wNoise( 
													data, detObjIdxOut, len,
													CFAR_CAGO,
													0, c2oneside+1, guardlen, noiselen,
		...
		case TEST_CACA:
			{
				printf("Test mmwavelib_cfarCadB_SOGO_wNoise, CFAR_CA.\n");
				numDetObj = mmwavelib_cfarCadB_SOGO_wNoise( 
													data, detObjIdxOut, len,
													CFAR_CA,
													0, c2oneside+1, guardlen, noiselen,
		...
		default: /*TEST_CA*/
			{
				printf("Test mmwavelib_cfarCadB_wNoise.\n");
				numDetObj = mmwavelib_cfarCadB_wNoise( 
													data, detObjIdxOut, len,
													0, c2oneside+1, guardlen, noiselen,
													noiseOut);
			}
		...
		if (numDetObj != numDetRef)
		{
			testFailed++;
			printf("FAILED! test type%d, number detected =%d, ref=%d\n", typeId, numDetObj,numDetRef);
		}

		/* Compare lib function's noise output with the reference */
		for ( i =0; i <numDetRef; i++)
		{
			if (detObjIdxRef[i] != detObjIdxOut[i])
			{
				testFailed++;
				printf("FAILED! test type%d, i=%d, detRef=%d, detOut=%d\n",
								typeId, i, detObjIdxRef[binIdx], detObjIdxOut[binIdx]);
			}
				
			if ( noiseRef[i] != noiseOut[i] )
			{
				testFailed++;
				printf("FAILED! test type%d, i=%d, noiseRef=%d, noiseOut=%d\n",
								typeId, i, noiseRef[binIdx], noiseOut[binIdx]);
				break;
			}
		}
	...
	if ( testFailed == 0 )
	{
        MCPI_setFeatureTestResult("CFAR with noise output functionality tests", MCPI_TestResult_PASS);
	}
	else
	{
		totalFailed += testFailed;
        MCPI_setFeatureTestResult("CFAR with noise output functionality tests", MCPI_TestResult_FAIL);
	}

} /* test_cfarnoiseoutput */
...
void benchmark_log2abs32_cfar()
{
	...
	*/
	...
	printf("length and cfarCadB_wNoise cycles:\n");
	for ( i=32; i<=1024; i=i*2 )
	{
		w_startTime1 = TSCL;
		mmwavelib_cfarCadB_wNoise(outptr, h_detObjIdxTemp,i, 1200,4,4,8, h_noiseOut);
		cycles = TSCL-w_startTime1;
		printf("%d\t%d\n", i, cycles);
	}

	printf("length and cfarCadBwrap_wNoise cycles:\n");
	for ( i=32; i<=1024; i=i*2 )
	{
		w_startTime1 = TSCL;
		mmwavelib_cfarCadBwrap_wNoise(outptr, h_detObjIdxTemp,i, 1200,4,4,8, h_noiseOut);
		cycles = TSCL-w_startTime1;
		printf("%d\t%d\n", i, cycles);
	}

	printf("length and cfarCadB_SOGO_wNoise (type CFAR_CA) cycles:\n");
	for ( i=32; i<=1024; i=i*2 )
	{
		w_startTime1 = TSCL;
		mmwavelib_cfarCadB_SOGO_wNoise(outptr, h_detObjIdxTemp,i, CFAR_CA, 1200,4,4,8,h_noiseOut);
		cycles = TSCL-w_startTime1;
		printf("%d\t%d\n", i, cycles);
	}

	printf("length and cfarCadB_SOGO_wNoise (type CFAR_CAGO) cycles:\n");
	for ( i=32; i<=1024; i=i*2 )
	{
		w_startTime1 = TSCL;
		mmwavelib_cfarCadB_SOGO_wNoise(outptr, h_detObjIdxTemp,i, CFAR_CAGO, 1200,4,4,8,h_noiseOut);
		cycles = TSCL-w_startTime1;
		printf("%d\t%d\n", i, cycles);
	}

	printf("length and cfarCadB_SOGO_wNoise (type CFAR_CASO) cycles:\n");
	for ( i=32; i<=1024; i=i*2 )
	{
		w_startTime1 = TSCL;
		mmwavelib_cfarCadB_SOGO_wNoise(outptr, h_detObjIdxTemp,i, CFAR_CASO, 1200,4,4,8,h_noiseOut);
		cycles = TSCL-w_startTime1;
		printf("%d\t%d\n", i, cycles);
	}
	
} /* benchmark log2abs32 and cfar*/
...
*/
    ...
    if ((fid = fopen ("testdata\\radarDemo_captured_ref.bin","rb")) == NULL)
    {
    ...
    if ((fid = fopen ("testdata\\radarDemo_captured_test.bin","rb")) == NULL)
    {
    ...
    if ((fid = fopen ("testdata\\radarDemo_multiObj_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_multiObj_test.bin","rb")) == NULL)
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
  ...
  if ( moretv )
  {
    ...
    if ((fid = fopen ("testdata\\radarDemo_multiObj_neg5_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_multiObj_neg5_test.bin","rb")) == NULL)
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_multiObj_neg15_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_multiObj_neg15_test.bin","rb")) == NULL)
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_test.bin","rb")) == NULL) //_log2absout_, _flatWin2D_fft2Dout_test.bin
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_neg10_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_neg10_test.bin","rb")) == NULL)
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_neg15_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarDemo_neg15_test.bin","rb")) == NULL)
    {
    ...
    if ((fid = fopen ("testdata\\radarCase1_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarCase1_test.bin","rb")) == NULL)
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarCase1_mulObj_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarCase1_mulObj_test.bin","rb")) == NULL)
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarCase1_512_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarCase1_512_test.bin","rb")) == NULL)
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
    }
    ...
    if ((fid = fopen ("testdata\\radarCase2_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
    ...
    read_refobj();
#if COMPARE_2DFFT
    if ((fid = fopen ("testdata\\radarCase2_2dfftout_test.bin","rb")) == NULL)
#else
    if ((fid = fopen ("testdata\\radarCase2_test.bin","rb")) == NULL)
#endif
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
	...
	*/
    ...
    if ((fid = fopen ("testdata\\radarCase2_mulObj_ref.bin","rb")) == NULL)
    {
        printf ("file open error: can not open ref file\n");
        exit(3);
    }
...
#if COMPARE_2DFFT
    if ((fid = fopen ("testdata\\radarCase2_mulObj_2dfftout_test.bin","rb")) == NULL)
#else
    if ((fid = fopen ("testdata\\radarCase2_mulObj_test.bin","rb")) == NULL)
#endif
    {
        printf ("file open error: can not open input data file\n");
        exit(3);
