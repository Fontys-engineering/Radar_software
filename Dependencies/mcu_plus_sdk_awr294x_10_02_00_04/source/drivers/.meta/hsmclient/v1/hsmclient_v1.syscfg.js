...
let hsmclient_module = {
    ...
    templates: {
        "/drivers/system/system_config.c.xdt": {
            driver_config: "/drivers/hsmclient/templates/hsmclient_config_v1.c.xdt",
	...
	config: [
        ...
        },
		{
			...
			description: "Total number of secure hosts. The can be only 1 secure hosts at a time. Make sure this field matches with HSM server configurations. ",
			default: 1,
			displayFormat: "dec",
            readOnly:true
		},
		{
			...
			displayName:"Secure Host Id",
			longDescription: "Secure_host_id for current core. Secure_host_id_0 is the primary secure core. HSMRt will send a bootnotify message to a primary secure core once it completes the initialization sequence and ready to accept requests.",
...
function validate(inst,report)
{
    ...
    if(remoteCoreInstArr.length > 1)
    {
        report.logError("Max number of secure cores is 1",inst);
    }
    else
    {
        for(let remoteCoreName of coreNames)
