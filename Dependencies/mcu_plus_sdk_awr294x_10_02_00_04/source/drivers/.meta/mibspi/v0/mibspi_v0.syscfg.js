...
function checkValidBitrate(instance, report, property, maxDiv) {

    let preipheralClkFreq = 200000000;

    if(common.getSocName() == "awr294x")
    {
        preipheralClkFreq = 150000000;
    }

    if(preipheralClkFreq%instance[property] != 0)
    {
        report.logError( `Invalid bitrate. Should be divisible by ${preipheralClkFreq}`,
        instance, property);
    }
    else if(preipheralClkFreq/(instance[property]+1) > (maxDiv+1))
    {
        report.logError( `Exceeds max "PRESCALE" value (255+1). Increase the bitrate.`,
        instance, property);
    }
}
...
function validate(inst, report) {
    ...
    checkValidBitrate(inst, report, "bitRate", 255);
